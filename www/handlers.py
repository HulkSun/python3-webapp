...
def text2html(text):
    lines = map(lambda s: '<p>%s</p>' % s.replace('&', '&amp;').replace('<','&lt;').replace('>','&gt;'), filter(lambda s: s.strip() != '', text.strip('\n')))
    return ' '.join(lines)
...
def get_blog(id):
    ...
    comments = yield from Comment.findAll('blog_id=?',
                                          [id],
...
def index(request):
    ...
    blogs = [
        Blog(id='1', name='first blog',
             summary='first summary dsfklasjdf lajsdlfkj alsdjf klasd ',
             create_at=time.time()-120),
        Blog(id='2', name='second blog',
             summary='second summary asdf asdfjkljsdfjowqiejr89uwfasdj ',
             create_at=time.time()-3600),
        Blog(id='3', name='third blog',
             summary='third summary saf asdfnkjdak ajksdhfkj alh8eyhr98rashdf',
             create_at=time.time()-4500)
    ]
    return {
